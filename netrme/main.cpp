...
int Running = 1;
LinuxArbotixPro linux_arbotix_pro("/dev/ttyUSB0");
ArbotixPro arbotix_pro(&linux_arbotix_pro);
  ...
  if(input_len > 0)
  {
    ...
    if(token != 0)
    {
      ...
      else if(strcmp(cmd, "mon") == 0)
      {
        ...
        arbotix_pro.m_bIncludeTempData = true;
        arbotix_pro.MakeBulkReadPacket();// force packet to get rebuilt
        while(!kbhit(true))
        {
          MonitorServos(&arbotix_pro);
          usleep(10000);
        }

        linuxMotionTimer.Stop();
        arbotix_pro.m_bIncludeTempData = false;
        arbotix_pro.MakeBulkReadPacket();// force packet to be rebuilt
      ...
      {
        if(num_param > 0)
          PlayCmd(&arbotix_pro, iparam[0]);
        else
          PlayCmd(&arbotix_pro, IndexPage());
      }
      else if(strcmp(cmd, "set") == 0)
      {
        if(num_param > 0)
          SetValue(&arbotix_pro, iparam[0]);
        else
          PrintCmd("Need parameter");
      }
      ...
      {
        OnOffCmd(&arbotix_pro, true, num_param, iparam, iparams);
      }
      else if(strcmp(cmd, "off") == 0)
      {
        OnOffCmd(&arbotix_pro, false, num_param, iparam, iparams);
      }
      ...
      {
        if(num_param > 0)
          GoCmd(&arbotix_pro, iparam[0]);
        else
          PrintCmd("Need parameter");
      }
      else if(strcmp(cmd, "poweroff") == 0)
      {
        arbotix_pro.DXLPowerOn(false);
      }
      else if(strcmp(cmd, "poweron") == 0)
      {
        arbotix_pro.DXLPowerOn(true);
      }
...
int main(int argc, char *argv[])
{
  ...
  if(MotionManager::GetInstance()->Initialize(&arbotix_pro) == false)
  {
