...
void ReadStep(ArbotixPro *arbotix_pro)
{
	int value;
	for(int id = 0; id < 31; id++)
	{
		...
		{
			if(arbotix_pro->ReadByte(id, AXDXL::P_TORQUE_ENABLE, &value, 0) == ArbotixPro::SUCCESS)
			{
				if(value == 1)
				{
					if(arbotix_pro->ReadWord(id, AXDXL::P_GOAL_POSITION_L, &value, 0) == ArbotixPro::SUCCESS)
		...
		else Step.position[id] = Action::INVALID_BIT_MASK;
	}
}
...
void DrawIntro(ArbotixPro *arbotix_pro)
{
	/*
...
void UpDownValue(ArbotixPro *arbotix_pro, int offset)
{
	SetValue(arbotix_pro, GetValue() + offset);
}
...
void SetValue(ArbotixPro *arbotix_pro, int value)
{
	int col;
	int row;
	if(bBeginCommandMode == true)
	...
	if( col == STP7_COL )
	{
		...
		else
		{
			if(value >= 0 && value <= AXDXL::MAX_VALUE)
			{
				...
				{
					int error;
					if(arbotix_pro->WriteWord(row + 1, AXDXL::P_GOAL_POSITION_L, value, &error) == ArbotixPro::SUCCESS)
					{
...
void ToggleTorque(ArbotixPro *arbotix_pro)
{
	...
	{
		if(arbotix_pro->WriteByte(id, AXDXL::P_TORQUE_ENABLE, 1, 0) != ArbotixPro::SUCCESS)
			return;

		int value;
		if(arbotix_pro->ReadWord(id, AXDXL::P_PRESENT_POSITION_L, &value, 0) != ArbotixPro::SUCCESS)
			return;
	...
	{
		if(arbotix_pro->WriteByte(id, AXDXL::P_TORQUE_ENABLE, 0, 0) != ArbotixPro::SUCCESS)
			return;
...
void MonitorServos(ArbotixPro *arbotix_pro)
{
	int value, j;
...
void PlayCmd(ArbotixPro *arbotix_pro, int pageNum)
{
...
void OnOffCmd(ArbotixPro *arbotix_pro, bool on, int num_param, int *list, char lists[30][10])
{
	...
	{
		for(int id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
			arbotix_pro->WriteByte(id, AXDXL::P_TORQUE_ENABLE, (int)on, 0);
	}
	else
	{
		for(int i = 0; i < num_param; i++)
		{
			...
			if(token != NULL)
			{
				...
				else
				{
					...
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(rl[id], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if(strcmp(token1, "ll") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(ll[id], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "ra") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(ra[id], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "la") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(la[id], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "h") == 0)
					{
						for(id = 0; id < 3; id++ )
							arbotix_pro->WriteByte(h[id], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if(list[i] >= JointData::ID_MIN && list[i] <= JointData::ID_MAX)
					{
						arbotix_pro->WriteByte(list[i], AXDXL::P_TORQUE_ENABLE, (int)on, 0);
					}
				}
			}
		}
...
void GoCmd(ArbotixPro *arbotix_pro, int index)
{
	if(index < 0 || index >= Action::MAXNUM_STEP)
	{
		PrintCmd("Invalid step index");
	...
	for(id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
	{
		...
		if(arbotix_pro->ReadWord(id, AXDXL::P_PRESENT_POSITION_L, &wStartPosition, 0) != ArbotixPro::SUCCESS)
		{
...
void ProcessPS3(Robot::ArbotixPro *arbotix_pro, int *apState)
{
	int num_param = 1;
	int iparam[30];
	char iparams[30][10];
	...
	if(PS3.key.R2 != 0)
	{
		...
		apState[0]++;
		apState[0] %= 2;
		OnOffCmd(arbotix_pro, apState[0] ? true : false, num_param, iparam, iparams);
	...
	if(PS3.key.R1 != 0)
	{
		...
		apState[1] %= 2;
		strcpy(iparams[0], "ra");
		OnOffCmd(arbotix_pro, apState[1] ? true : false, num_param, iparam, iparams);
	...
	if(PS3.key.L2 != 0)
	{
		...
		apState[2] %= 2;
		strcpy(iparams[0], "ll");
		OnOffCmd(arbotix_pro, apState[2] ? true : false, num_param, iparam, iparams);
	...
	if(PS3.key.L1 != 0)
	{
		...
		apState[3] %= 2;
		strcpy(iparams[0], "la");
		OnOffCmd(arbotix_pro, apState[3] ? true : false, num_param, iparam, iparams);
	...
	if(PS3.key.Up != 0)
	{
		...
		apState[4] %= 2;
		strcpy(iparams[0], "h");
		OnOffCmd(arbotix_pro, apState[4] ? true : false, num_param, iparam, iparams);
