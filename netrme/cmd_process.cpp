	...
	if( col == STP7_COL )
	{
		...
		else
		{
			if(value >= 0 && value <= MX28::MAX_VALUE)
			{
				...
				{
					int error;
					if(arbotix_pro->WriteWord(row + 1, MX28::P_GOAL_POSITION_L, value, &error) == ArbotixPro::SUCCESS)
					{
...
void ToggleTorque(ArbotixPro *arbotix_pro)
{
	...
	{
		if(arbotix_pro->WriteByte(id, MX28::P_TORQUE_ENABLE, 1, 0) != ArbotixPro::SUCCESS)
			return;
	...
	{
		if(arbotix_pro->WriteByte(id, MX28::P_TORQUE_ENABLE, 0, 0) != ArbotixPro::SUCCESS)
			return;
...
void OnOffCmd(ArbotixPro *arbotix_pro, bool on, int num_param, int *list, char lists[30][10])
{
	...
	{
		for(int id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
			arbotix_pro->WriteByte(id, MX28::P_TORQUE_ENABLE, (int)on, 0);
	}
	else
	{
		for(int i = 0; i < num_param; i++)
		{
			...
			if(token != NULL)
			{
				...
				else
				{
					...
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(rl[id], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if(strcmp(token1, "ll") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(ll[id], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "ra") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(ra[id], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "la") == 0)
					{
						for(id = 0; id < 6; id++ )
							arbotix_pro->WriteByte(la[id], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if (strcmp(token1, "h") == 0)
					{
						for(id = 0; id < 3; id++ )
							arbotix_pro->WriteByte(h[id], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
					else if(list[i] >= JointData::ID_MIN && list[i] <= JointData::ID_MAX)
					{
						arbotix_pro->WriteByte(list[i], MX28::P_TORQUE_ENABLE, (int)on, 0);
					}
				}
			}
		}
...
void GoCmd(ArbotixPro *arbotix_pro, int index)
{
	...
	for(id = JointData::ID_MIN; id <= JointData::ID_MAX; id++)
	{
		...
		if(arbotix_pro->ReadWord(id, MX28::P_PRESENT_POSITION_L, &wStartPosition, 0) != ArbotixPro::SUCCESS)
		{
