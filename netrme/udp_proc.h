...
void PopulateStepCurrent( ArbotixPro *arbotix_pro, Action::STEP *CurrentStep )
{
	int value;
...
{
	if ( arbotix_pro->WriteByte(ServoID, AXDXL::P_TORQUE_ENABLE, on, 0) != ArbotixPro::SUCCESS  )
	{
		printf( "Warning: SetTorque[%d] did not return success for servo: %d\r\n", on, ServoID );
	}
#ifdef DEBUG
	int value;
	if(arbotix_pro->ReadWord(ServoID, AXDXL::P_PRESENT_POSITION_L, &value, 0) != ArbotixPro::SUCCESS)
	{
...
void TorqueAllServos(ArbotixPro *arbotix_pro, char * input)
{
	int on;
	if(input[0] == '1')
	{
...
void GotoPose( ArbotixPro *arbotix_pro, char* input )
{
	...
	PopulateStepCurrent(arbotix_pro, &CurrentStep);
...
void GetCurrentPose( ArbotixPro *arbotix_pro, char* response )
{
	Action::STEP CurrentStep;
	PopulateStepCurrent( arbotix_pro, &CurrentStep );
...
void SetServo( ArbotixPro *arbotix_pro, char* input )
{
	//SETSERVO:1:774
	input += 9;
	...
	while( pch != 0 )
	{
		...
		int error;
		if ( arbotix_pro->WriteWord(index, AXDXL::P_GOAL_POSITION_L, value, &error) != ArbotixPro::SUCCESS )
		{
...
void SetServoSpeeds( ArbotixPro *arbotix_pro, char* input )
{
	//SETSPEED:0-1023
	input += 9;
	...
	if ( pch != 0 )
	{
		...
		int error;
		if ( arbotix_pro->WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_MOVING_SPEED_L, speed, &error) != ArbotixPro::SUCCESS )
		{
...
void PlayPage( ArbotixPro *arbotix_pro, char* input )
{
	...
	GetCurrentPose( arbotix_pro, tempMessage );
...
void UDP_Command_Handler( char * p_udpin )
{
	...
	{
		TorqueAllServos( &arbotix_pro, p_udpin + 10 );
	...
	else if( memcmp( p_udpin, "TORQUE", 6 ) == 0 )
	{
		...
		tok = strtok( 0, " :" );
		int onoff = atoi( tok );
		SetTorque( &arbotix_pro, servoid, onoff );
		GetCurrentPose( &arbotix_pro, tempMessage );
	}
	else if( memcmp( p_udpin, "GOTOPOSE:", 9 ) == 0 )
	{
		GotoPose( &arbotix_pro, p_udpin );
		return;
	}
...
//NOTE: For safety, especially during debugging, make sure motor torque is on before playing!
	...
	{
		SetServo( &arbotix_pro, p_udpin );
		return;
	}
	else if( memcmp( p_udpin, "SETSPEED:", 9 ) == 0 )
	{
		SetServoSpeeds( &arbotix_pro, p_udpin );
		return;
	}
...
void UDP_Control_Handler( char * p_udpin )
{
	...
	if ( memcmp( p_udpin, "BIND", 4 ) == 0 )
	{
		...
		if ( arbotix_pro.WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_TORQUE_LIMIT_L, 684, 0) != ArbotixPro::SUCCESS  )
		{
			printf( "Warning: TODO\r\n");
		}
		if ( arbotix_pro.WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_MOVING_SPEED_L, 256, 0) != ArbotixPro::SUCCESS )
		{
			printf( "Warning: TODO\r\n");
		}
	}
